...
# Load data
data <- read.csv("Pooled R data on egg mass per snail.csv")
...
# Filter only significant comparisons (exclude "ns")
sig_dunn_res <- dunn_res %>%
  filter(p.adj.signif != "ns") %>%
  mutate(y.position = c(9.0, 9.7, 10.7))  # Adjust heights if needed
...
# Plot
ggplot(data2, aes(x = Diet, y = avg_egg_mass_per_egg_layer, color = Diet)) + 
  geom_jitter(width = 0.2, alpha = 0.6, size = 3) +
  scale_color_manual(values = diet_colors) +
  stat_summary(fun = mean, geom = "point", size = 5, color = "black") +
  stat_pvalue_manual(sig_dunn_res, label = "p.adj.signif", tip.length = 0.02, size = 10) +
  ...
  theme(
    ...
    panel.border = element_rect(linewidth = 2.0, fill = NA),
    legend.position = "none"
  )

# Save the final figure
ggsave("Average egg mass per egg-laying snail.png", width = 6.5, height = 6.5, dpi = 300)
