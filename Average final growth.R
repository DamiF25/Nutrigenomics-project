...
# Keep only significant comparisons and assign stars
p_to_star <- function(p) {
           ifelse(p <= 0.0001, "****",  # ≤ 1e-4
                ifelse(p <= 0.001, "***", # ≤ 1e-3
                       ifelse(p <= 0.01, "**", # ≤ 1e-2
                              ifelse(p <= 0.05, "*", ""))))
}
significant_comparisons <- dunn_results %>%
  ...
  mutate(
    p.adj = P.adj,
    label = p_to_star(p.adj)  # Convert p-values to stars
  )
...
ggsave("Final growth_Dunn_sig2.png", width = 9, height = 7, dpi = 300)
