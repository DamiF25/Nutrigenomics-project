...
#NOTE: Save all files in the same folder
...
otu_table <- as.data.frame(merged_df[, -1]) # Drop 'name' column
...
dev.off() # This automatically saves the figure into the local folder directory
...
# Facet box plot
ggplot(alpha_long, aes(x = Group, y = Value, fill = Group, color = Group)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.75, width = 0.4) +
  geom_jitter(aes(color = Group), width = 0.2, size = 1.6, alpha = 0.8) +
  ...
  scale_color_manual(values = c("FF" = "#f00000", "FL" = "#000000", "SG" = "#0f00f7")) +
  theme_test() +
  theme(
