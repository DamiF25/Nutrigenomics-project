...
# Define colors
diet_colors <- c("DL" = "#54BAA4", "FF" = "#DB6A24", "SG" = "#D97BE0", "WL+FF" = "#FFD000",
                 "WL+SG" = "#D4BA7F")

# Box plot with significant comparisons
boxplot <- ggplot(data, aes(x = Diet, y = Embryos)) +
  geom_boxplot(aes(color = Diet, fill = Diet), outlier.shape = NA, size = 1.5, alpha = 0.5) +  # Box outline.
  ...
  stat_pvalue_manual(
    ...
    label = "label", size = 10,
    y.position = seq(max(data$Embryos) + 140, by = 1, length.out = nrow(tukey_annotations)),
    step.increase = 0.1
  ) +
...
ggsave("Average embryo count2.png", width = 7, height = 6.5, dpi = 300)
